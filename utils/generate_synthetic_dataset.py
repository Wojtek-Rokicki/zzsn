...
def generate_one_point(config, previous):
    ...
    if previous is None:     # The first point is always accepted
        return np.array([pos[0], pos[1], pos[2], 0, valency])[None, :], 0 # [None, :] or [:, None] trick for expanding dimension
...
def generate_dataset(save_to: str):
    ...
    # all_sets - lista grafów o różnej ilości węzłów
    # rozkład grafów według ilości węzłów
    for i, set in enumerate(all_sets):
        n_dist[set.shape[0]] += 1
        # oznaczenie do którego grafu należy węzeł (atom)
